...
class TableauFabricMappings:
    ...
    def __init__(self):
        ...
        # Create reverse mapping for validation (Fabric -> Tableau)
        self.reverse_mappings = {
            value.upper(): key.upper() 
            for key, value in self._BASE_FUNCTION_MAPPINGS.items()
        }
        
        # Special syntax patterns that need regex handling
        self.special_patterns = {
            # Tableau boolean literals -> Fabric equivalents
            'TRUE': '1',
            'FALSE': '0',
            
            # Tableau comment syntax
            '--': '--',  # Same
            '//': '--',  # Tableau // -> Fabric --
            
            # String concatenation
            '+': '+',  # Same in both (SQL Server style)
        }
...
tableau_fabric_mappings = TableauFabricMappings()
